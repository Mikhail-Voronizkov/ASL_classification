...
*/
#include <alphabet_classification_inferencing.h>
...
const char* password = "0927978199";
...
void logMemory() {
  ...
  Serial.printf("\nUsed PSRAM: %d", ESP.getPsramSize() - ESP.getFreePsram());
...
void classify()
{
  ...
  for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
    ...
    if (result.classification[ix].value > max){
      ...
      tmp = result.classification[ix].label;
      if(tmp == "nothing"){
        tmp = "";
      }
    }
